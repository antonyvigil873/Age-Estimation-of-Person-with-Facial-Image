...
def Model(X, Y, test_x, test_y, BS):
    ...
    model.fit(X, Y, epochs=10, batch_size=BS, steps_per_epoch=5, validation_data=(test_x, test_y))
    pred = model.predict(test_x)
    return pred


def Model_CNN(train_data, train_target, test_data, test_target, BS=None):
    print('Model CNN')
    if BS is None:
        BS = 4
    IMG_SIZE = 32
